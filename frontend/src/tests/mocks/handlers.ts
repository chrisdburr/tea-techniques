...
export const handlers = [
  ...
  http.get("*/api/techniques", ({ request }) => {
    ...
    const assuranceGoals =
      url.searchParams.get("assurance_goals") ||
      url.searchParams.get("assurance_goals[]");
    ...
    return HttpResponse.json({
      ...
      next:
        endIndex < filteredTechniques.length
          ? `/api/techniques/?page=${page + 1}`
          : null,
  ...
  http.post("*/api/techniques", async ({ request }) => {
    ...
    if (
      ...
      (data.complexity_rating < 1 || data.complexity_rating > 5)
    ) {
      return HttpResponse.json(
        {
          error: true,
          message: "Validation failed",
          details: {
            complexity_rating: ["Complexity rating must be between 1 and 5."],
          },
        },
        { status: 400 },
      );
  ...
  http.get("*/api/assurance-goals/:id", ({ params }) => {
    const { id } = params;
    const goal = mockAssuranceGoals.find(
