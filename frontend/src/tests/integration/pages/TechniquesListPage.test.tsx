...
const TechniquesListPage = ({
  initialTechniques,
}: { initialTechniques?: typeof mockTechniques } = {}) => {
  const [filteredTechniques, setFilteredTechniques] = React.useState(
    initialTechniques || mockTechniques,
  );

  console.log(
    "TechniquesListPage rendering with",
        ...
        <input
          ...
          onChange={(e) => {
            ...
            if (!query) {
              ...
              const filtered = mockTechniques.filter(
                (t) =>
...
describe("Techniques List Page Integration", () => {
  ...
  describe("Page Loading and Initial State", () => {
    it("renders the page with initial techniques loaded", async () => {
      ...
      const page = container.querySelector(
        '[data-testid="techniques-list-page"]',
      );
      ...
      for (const technique of mockTechniques) {
        ...
        const hasName = Array.from(nameElements).some(
          (el) => el.textContent?.includes(technique.name.split(" ")[0]),
        );
    ...
    it("displays loading state while fetching techniques", async () => {
      ...
      const nameElements = container.querySelectorAll("h3");
      const hasFirstTechnique = Array.from(nameElements).some(
        (el) => el.textContent?.includes(mockTechniques[0].name.split(" ")[0]),
      );
  ...
  describe("Filtering Functionality", () => {
    it("filters techniques by assurance goal", async () => {
      ...
      const goalSelect = container.querySelector(
        'select[aria-label="Filter by assurance goal"]',
      );
      if (goalSelect) {
    ...
    it("filters techniques by complexity rating", async () => {
      ...
      const complexitySelect = container.querySelector(
        'select[aria-label="Filter by complexity"]',
      );
      if (complexitySelect) {
  ...
  describe("Technique Card Interactions", () => {
    it("navigates to technique detail when card is clicked", async () => {
      ...
      if (techniqueLink) {
        ...
        expect(techniqueLink).toHaveAttribute(
          "href",
          `/techniques/${mockTechniques[0].slug}`,
        );
      }
    });
    ...
    it("handles card hover states appropriately", async () => {
      ...
      const firstCard = container.querySelector(
        `[data-testid="technique-card-${mockTechniques[0].slug}"]`,
      );
  ...
  describe("Performance and Optimization", () => {
    it("loads efficiently with many techniques", async () => {
      ...
      const { container } = renderWithProviders(
        <TechniquesListPage initialTechniques={largeTechniquesList} />,
      );
  ...
  describe("Accessibility", () => {
    ...
    it("provides appropriate ARIA labels", async () => {
      ...
      expect(
        container.querySelector('select[aria-label="Filter by tag"]'),
      ).toBeInTheDocument();
      expect(
        container.querySelector('select[aria-label="Filter by complexity"]'),
      ).toBeInTheDocument();
    });
      ...
      if (searchInput) {
        ...
        await waitFor(() => {
          const statusRegion =
            ...
            container.querySelector('[aria-label*="results"]');
