...
export class ApiMocker {
  ...
  private handlers: Map<string, (url: string, options?: RequestInit) => Promise<Response>>
  ...
  onGet(pattern: string | RegExp, handler: () => Promise<Response> | Response) {
    this.handlers.set(`GET:${pattern.toString()}`, async (url, options) => {
      if (this.matchesPattern(url, pattern) && (!options?.method || options.method === 'GET')) {
  ...
  onDelete(pattern: string | RegExp, handler: () => Promise<Response> | Response) {
  ...
  execute() {
    this.mockFetch.mockImplementation(async (url: string, options?: RequestInit) => {
      ...
      return createApiResponse({ error: `No handler found for ${method} ${url}` }, 404)
    })
  }
...
export const mockApiScenarios = {
  ...
  emptyState: () => {
    const mocker = createApiMocker()
    mocker
      .onGet(API_ENDPOINTS.TECHNIQUES, () => createApiResponse({ count: 0, results: [] }))
  ...
  slowResponses: () => {
    ...
    mocker
      .onGet(API_ENDPOINTS.TECHNIQUES, () => 
        new Promise(resolve => setTimeout(() => resolve(mockApiHandlers.getTechniques()), 2000))
      )
      .execute()
    return mocker
  },
