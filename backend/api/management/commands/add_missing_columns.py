...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if db_engine != "sqlite":
            self.stdout.write(
                self.style.WARNING(
                    f"This command is intended for SQLite databases only. Current database is {db_engine}."
                )
            )
            return
        ...
        try:
            ...
            self.stdout.write(
                f"Found columns in technique table: {', '.join(column_names)}"
            )
            ...
            if "applicable_models" not in column_names:
                self.stdout.write(
                    self.style.WARNING(
                        "Adding missing 'applicable_models' column to technique table..."
                    )
                )

                # Add the missing column
                cursor.execute(
                    "ALTER TABLE technique ADD COLUMN applicable_models text NULL"
                )
                self.stdout.write(
                    self.style.SUCCESS(
                        "Added 'applicable_models' column to technique table"
                    )
                )
            else:
                self.stdout.write(
                    self.style.SUCCESS(
                        "Column 'applicable_models' already exists in technique table"
                    )
                )
            ...
            if "applicable_models" in column_names:
                self.stdout.write(
                    self.style.SUCCESS(
                        "Column 'applicable_models' is now present in the schema"
                    )
                )
            else:
                self.stdout.write(
                    self.style.ERROR("Failed to add 'applicable_models' column")
                )

        except Exception as e:
            self.stdout.write(
                self.style.ERROR(f"Error modifying database schema: {str(e)}")
            )
            connection.rollback()
            return
