// src/app/techniques/[slug]/TechniqueDetailClient.tsx
"use client";
...
function TechniqueExampleUseCases({
  ...
  if (!useCases || useCases.length === 0) {
    return (
      <p className="text-muted-foreground">No example use cases specified.</p>
    );
  }
...
function RelatedTechniques({
  techniqueSlugs,
  initialTechniqueNames,
}: {
  techniqueSlugs: string[];
  initialTechniqueNames?: Array<{
    slug: string;
    name: string;
    acronym?: string;
  }>;
}) {
  const { techniques, isLoading, isError } =
    useMultipleTechniqueNames(techniqueSlugs);

  // Use initial data if provided, otherwise use hook data
  const finalTechniques = initialTechniqueNames || techniques;
  const finalIsLoading = initialTechniqueNames ? false : isLoading;

  if (!techniqueSlugs || techniqueSlugs.length === 0) {
    return (
      <p className="text-muted-foreground">No related techniques specified.</p>
    );
  }
  ...
  if (isError) {
    return (
      <p className="text-sm text-muted-foreground">
        Error loading related techniques. They may not exist or you may not have
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {finalTechniques
          ?.filter((technique): technique is NonNullable<typeof technique> =>
            Boolean(technique),
          )
          .map((technique) => (
            <Link
...
export default function TechniqueDetailClient({
  ...
  const finalTechnique = initialTechnique || technique;
  const finalIsLoading = initialTechnique ? false : isLoading;
  ...
  useEffect(() => {
    if (finalTechnique) {
      console.log("Technique data loaded:", finalTechnique);
    }
  }, [finalTechnique]);
  ...
  if (error || !finalTechnique) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        <h1 className="text-2xl font-bold mb-4">Error Loading Technique</h1>
        <p className="text-muted-foreground mb-8">
          There was an error loading this technique. It may not exist or you may
  ...
  const applicableModels = getApplicableModels(finalTechnique.tags);
  const dataTypes = getDataTypes(finalTechnique.tags);
  const lifecycleStages = getLifecycleStages(finalTechnique.tags);
  const groupedTags = groupTagsByPrefix(finalTechnique.tags);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <Section title="Example Use Cases">
            <TechniqueExampleUseCases
              useCases={finalTechnique.example_use_cases}
            />
          </Section>

          <Section title="Limitations">
            <TechniqueLimitations limitations={finalTechnique.limitations} />
          </Section>

          {/* Related Techniques Section */}
          {finalTechnique.related_techniques &&
            finalTechnique.related_techniques.length > 0 && (
              <Section title="Related Techniques">
                <RelatedTechniques
                  techniqueSlugs={finalTechnique.related_techniques}
                />
              </Section>
            )}

          <Section title="Resources" noBorder={true}>
            <TechniqueResources resources={finalTechnique.resources} />
          </Section>
        </div>
        ...
        <div>
          <Card className="sticky top-4 shadow-sm">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-3">
                  {finalTechnique.assurance_goals.map((goal) => (
                    <div
                      key={goal.id}
              ...
              {/* Ratings */}
              {finalTechnique.complexity_rating ? (
                <div className="space-y-2">
                  <h3 className="text-sm font-medium flex items-center">
                    Complexity
                  </h3>
                  <StarRating
                    rating={finalTechnique.complexity_rating}
                    maxRating={5}
                    className="text-amber-400"
                    aria-label={`Complexity rating: ${finalTechnique.complexity_rating} out of 5`}
                  />
                </div>
              ) : null}

              {finalTechnique.computational_cost_rating ? (
                <div className="space-y-2">
                  <h3 className="text-sm font-medium flex items-center">
                    Computational Cost
                  </h3>
                  <StarRating
                    rating={finalTechnique.computational_cost_rating}
                    maxRating={5}
                    className="text-amber-400"
                    aria-label={`Computational cost rating: ${finalTechnique.computational_cost_rating} out of 5`}
                  />
                </div>
              ) : null}
                              ...
                              {subcategoryTags.map((tag) => {
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <span>
                                      {formatTagDisplay(subcategories)}
                                    </span>
                                  </Badge>
                                );
                              })}
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {tags.map((tag) => (
                              ...
                              >
                                <Tag className="h-3 w-3" />
                                <span>
                                  {formatTagDisplay(getTagValue(tag.name))}
                                </span>
                              </Badge>
                            ))}
                          </div>
            ...
            {/* Edit section - only shown in API mode when editing is enabled */}
            {featureFlags.enableEditing && (
              <CardFooter className="flex flex-col items-stretch pt-4 pb-4 border-t">
                ...
                >
                  ...
                  />
                  <p className="text-sm text-muted-foreground">
                    Authentication is required to edit a technique. Not
                    implemented yet.
                  </p>
                </div>
                <Button
                ...
                >
                  <Link href="#" aria-label="Edit technique (disabled)">
