...
class HTTPErrorTests(APITestCase):
    ...
    def test_custom_error_response_format(self):
        ...
        error_scenarios = [
            # 404 error
            (reverse('technique-detail', kwargs={'pk': 99999}), 'get'),
            # 400 error
...
class ValidationDetailTests(APITestCase):
    ...
    def test_date_validation_in_resources(self):
        ...
        for invalid_date in invalid_dates:
            ...
            if response.status_code == status.HTTP_400_BAD_REQUEST:
                ...
                date_error_present = any(term in error_message for term in [
                    'date format', 'invalid date', 'date'
                ])
    ...
    def test_string_length_validation(self):
        ...
        if response.status_code == status.HTTP_400_BAD_REQUEST:
            ...
            self.assertTrue(
                'name' in response.data['details'] or 
                'description' in response.data['details']
            )
...
class CustomExceptionHandlerTests(APITestCase):
    ...
    def test_exception_handler_logging(self):
        ...
        with patch('api.utils.logger') as mock_logger:
            ...
            exception_logged = any('Http404' in call or 'NotFound' in call or 'DoesNotExist' in call for call in log_calls)
