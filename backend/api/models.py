...
class AssuranceGoal(models.Model):
    ...
    def clean(self):
        super().clean()
        if (
            AssuranceGoal.objects.filter(name__iexact=self.name)
            .exclude(id=self.id)
            .exists()
        ):
            raise ValidationError(
                {"name": "AssuranceGoal with this name already exists."}
            )
    ...
    def clean(self):
        ...
        if (
            Category.objects.filter(
                name__iexact=self.name, assurance_goal=self.assurance_goal
            )
            .exclude(id=self.id)
            .exists()
        ):
            raise ValidationError(
                {
                    "name": "Category with this name already exists for this assurance goal."
                }
            )
...
class SubCategory(models.Model):
    ...
    def clean(self):
        super().clean()
        if (
            SubCategory.objects.filter(name__iexact=self.name, category=self.category)
            .exclude(id=self.id)
            .exists()
        ):
            raise ValidationError(
                {"name": "SubCategory with this name already exists for this category."}
            )
    ...
    def get_by_natural_key(cls, name, category_name, assurance_goal_name):
        category = Category.objects.get_by_natural_key(
            category_name, assurance_goal_name
        )
...
class AttributeType(models.Model):
    ...
    required_for_goals = models.ManyToManyField(
        AssuranceGoal, related_name="required_attribute_types", blank=True
    )
...
class Technique(models.Model):
    ...
    assurance_goals = models.ManyToManyField(
        AssuranceGoal, through="TechniqueAssuranceGoal", related_name="techniques"
    )
    categories = models.ManyToManyField(
        Category, through="TechniqueCategory", related_name="techniques"
    )
    ...
    def clean(self):
        super().clean()
        if (
            Technique.objects.filter(name__iexact=self.name)
            .exclude(id=self.id)
            .exists()
        ):
            raise ValidationError(
                {"name": "A technique with this name already exists."}
            )
...
class TechniqueAttribute(models.Model):
    technique = models.ForeignKey(
        Technique, on_delete=models.CASCADE, related_name="attributes"
    )
...
class TechniqueRelationship(models.Model):
    technique_from = models.ForeignKey(
        Technique, related_name="related_from", on_delete=models.CASCADE
    )
    technique_to = models.ForeignKey(
        Technique, related_name="related_to", on_delete=models.CASCADE
    )
    relationship_type = models.CharField(max_length=50, default="similar")
