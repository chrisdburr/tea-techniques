...
interface Technique {
  ...
  resources?: Resource[];          // Academic/industry sources
  complexity_rating?: number;      // 1-5 scale
  computational_cost_rating?: number; // 1-5 scale
  acronym?: string;               // Short identifier
  related_techniques?: string[];   // Slugs of related techniques
}
...
interface Resource {
  title: string;
  url: string;
  source_type: string;   // e.g., "technical_paper", "tutorial"
  authors?: string[];
...
- **resources**: Optional. Academic papers, documentation, tutorials, or software packages
- **complexity_rating**: Optional. A 1-5 rating indicating implementation difficulty
- **computational_cost_rating**: Optional. A 1-5 rating indicating resource requirements
- **acronym**: Optional. Common abbreviation for the technique
- **related_techniques**: Optional. List of slugs for conceptually similar techniques
