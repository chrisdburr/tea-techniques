...
describe('TechniqueCard', () => {
  ...
  describe('Basic Rendering', () => {
    ...
    it('displays View Details button as a link', () => {
      ...
      const link = container.querySelector('a[href="/techniques/test-technique"]')
  ...
  describe('Assurance Goals', () => {
    it('displays assurance goal icons with tooltips', () => {
      ...
      const explainabilityElement = container.querySelector('[title="Explainability"]')
  ...
  describe('Description Truncation', () => {
    it('truncates long descriptions and adds ellipsis', () => {
      const longDescription = 'This is a very long description that should be truncated because it exceeds the maximum length limit for technique card descriptions. It continues on and on with lots of detailed information that would make the card too tall and affect the layout of the grid system.'
      ...
      const descriptionElement = container.querySelector(`[title="${longDescription}"]`)
      if (descriptionElement) {
        ...
        expect(container.textContent).toContain(longDescription.substring(0, 50))
      }
    })
  ...
  describe('Title Formatting', () => {
    it('correctly formats title with acronym', () => {
      ...
      expect(container.textContent).toContain('Machine Learning Interpretability (MLI)')
    })
    ...
    it('handles long titles gracefully', () => {
      ...
      const titleElement = container.querySelector('[title*="Very Long Technique Name"]')
      expect(titleElement).toBeInTheDocument()
      if (titleElement) {
        expect(titleElement).toHaveAttribute('title', 'Very Long Technique Name That Might Cause Layout Issues (VLTNMCLI)')
      }
    })
  })
  ...
  describe('Accessibility', () => {
    ...
    it('provides title attributes for goal icons', () => {
      ...
      const explainabilityIcon = container.querySelector('[title="Explainability"]')
  ...
  describe('Edge Cases', () => {
    it('handles special characters in technique name', () => {
      ...
      const { container } = renderComponent(technique)
      expect(container.textContent).toContain('Technique with Special Characters: αβγ & émojis 🤖')
    })
