...
export default function CategoriesPage() {
  ...
  useEffect(() => {
    if (assuranceGoalsData?.results) {
      const goal = assuranceGoalsData.results.find(
  ...
  const goalTechniques = React.useMemo(() => {
    ...
    if (techniquesData.results.length > 0) {
      console.log(
        "First technique structure:",
        JSON.stringify(techniquesData.results[0], null, 2),
      );
      console.log(
        "First technique goals:",
        techniquesData.results[0].assurance_goals,
      );
    }
    ...
    const filteredResults = techniquesData.results.filter(
      (technique: Technique) => {
        ...
        if (techniquesData.results.indexOf(technique) < 3) {
          console.log(
            `Technique ${technique.slug} (${technique.name}) goals:`,
        ...
        const hasMatchingGoal =
          ...
          technique.assurance_goals.some((goal) => {
            ...
            const matches =
              goal && typeof goal === "object" && goal.name === selectedGoal;
            return matches;
          });
    ...
    console.log(
      `Found ${filteredResults.length} techniques for ${selectedGoal}`,
    );
    return {
      ...techniquesData,
  ...
  const goalDescriptions: Record<string, string> = {
    Explainability:
      "Techniques that aim to make AI models more interpretable and their decisions more understandable to humans. These techniques help identify how models transform inputs into outputs, which features are most important for predictions, and why specific decisions are made.",
    Fairness:
      "Techniques that assess and mitigate bias in AI systems to ensure equitable outcomes across diverse demographic groups. These approaches help identify, measure, and reduce discrimination and unfairness in algorithmic decision-making.",
    Security:
      "Techniques that protect AI systems from adversarial attacks, vulnerabilities, and other security threats. These methods enhance the robustness of AI systems against manipulation and unauthorized exploitation.",
    Privacy:
      "Techniques that help preserve data privacy and confidentiality when developing or deploying AI systems. These approaches enable machine learning while protecting sensitive information from exposure or inference.",
    Reliability:
      "Techniques that ensure AI systems perform consistently and as expected across various conditions and environments. These methods improve robustness, error detection, and failure prevention in AI applications.",
    Safety:
      "Techniques that prevent harm to humans, environments, and other systems from AI operations. These approaches focus on identifying and mitigating potential risks, ensuring AI systems operate within safe boundaries and prevent unintended consequences.",
    Transparency:
      "Techniques that promote openness about how AI systems are developed, governed, and maintained. These approaches focus on documenting project governance, decision-making processes, and development methodologies to build trust and enable effective oversight.",
  };
  ...
  const createFilterUrl = (
    ...
    return `/techniques?${filterType}=${encodeURIComponent(
      ...
      <div className="space-y-12">
        <section className="space-y-6">
          <h1 className="text-3xl font-bold text-center">
            TEA Techniques Categories
          </h1>
          <p className="text-lg text-muted-foreground text-center max-w-3xl mx-auto">
            Explore techniques by assurance goals, categories, or model
            dependencies to find the right approach for your AI assurance needs.
          </p>
        </section>
          ...
          >
            <TabsList className="flex flex-wrap gap-2">
              {!isLoadingGoals &&
                ...
                assuranceGoalsData.results.map((goal: AssuranceGoal) => (
                  <TabsTrigger
                    key={goal.id}
                    value={goal.name}
              ...
              assuranceGoalsData.results.map((goal: AssuranceGoal) => (
                ...
                >
                  ...
                  <div className="bg-muted/30 p-6 rounded-lg">
                    <p className="text-lg">
                      {goalDescriptions[goal.name] || goal.description}
                    </p>
                  </div>
                  ...
                  <div className="space-y-4">
                    <h3 className="text-xl font-semibold">
                      Example {goal.name} Techniques
                    </h3>

                    {isLoadingTechniques ? (
                      <p>Loading examples...</p>
                    ) : goalTechniques?.results &&
                      goalTechniques.results.length > 0 ? (
                          ...
                          .map((technique: Technique) => (
                            ...
                            >
                              <CardHeader>
                                ...
                                <CardDescription>
                                  Complexity:{" "}
                                  {technique.complexity_rating || "N/A"}
                                </CardDescription>
                              </CardHeader>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {createPlaceholderTechniques(selectedGoal).map(
                          (technique: Technique) => (
                            <Card
                              key={technique.slug}
                              className="h-full flex flex-col"
                    ...
                    <div className="flex justify-center mt-6">
                      <Button asChild size="lg">
                        <Link
                          href={createFilterUrl("assurance_goal", selectedGoal)}
                        >
            ...
            <div className="max-w-3xl mx-auto space-y-6">
              <div className="space-y-2">
                ...
                <p className="text-lg ml-7">
                  ...
                  most important for your system—explainability, fairness,
                  security, etc.
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <p className="text-lg ml-7">
                  Some techniques require access to model internals, while
                  others work with any model as a black box. Choose based on
                  your access level.
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <p className="text-lg ml-7">
                  Within each assurance goal, different categories address
                  specific aspects of the problem. Browse these to narrow your
                  search.
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <p className="text-lg ml-7">
                  Each technique has different complexity and computational
                  requirements. Consider your resources and constraints.
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <p className="text-lg ml-7">
                  Often, the best approach is to use multiple techniques
                  together to address different aspects of AI assurance.
                </p>
              </div>
            </div>
          </div>
