    ...
    def test_authentication_required_endpoints(self):
        ...
        for endpoint_name, method in endpoints_requiring_auth:
            if 'detail' in endpoint_name:
                ...
                url = reverse(endpoint_name, kwargs={'pk': technique.id})
            else:
...
class PermissionTests(APITestCase):
    ...
    def test_read_permissions_anonymous(self):
        ...
        read_endpoints = [
            ...
            reverse('technique-detail', kwargs={'pk': self.technique.id}),
    ...
    def test_delete_permissions(self):
        ...
        techniques = [
            TechniqueFactory(name=f'Delete Test {i}') for i in range(3)
        ]
        
        # Test as anonymous user
        self.client.logout()
        url = reverse('technique-detail', kwargs={'pk': techniques[0].id})
        ...
        # Test as regular user
        self.client.force_authenticate(user=self.regular_user)
        url = reverse('technique-detail', kwargs={'pk': techniques[1].id})
        response = self.client.delete(url)
        # Might be allowed or forbidden
        
        # Test as admin
        self.client.force_authenticate(user=self.admin_user)
        url = reverse('technique-detail', kwargs={'pk': techniques[2].id})
        response = self.client.delete(url)
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        
        # Verify deletion
        self.assertFalse(Technique.objects.filter(id=techniques[2].id).exists())
    ...
    def test_xss_protection_in_responses(self):
        ...
        for xss in xss_attempts:
            ...
            if create_response.status_code == status.HTTP_201_CREATED:
                # Retrieve it
                technique_id = create_response.data['id']
                retrieve_url = reverse('technique-detail', kwargs={'pk': technique_id})
