...
const DataList = React.forwardRef<HTMLDListElement, DataListRootProps>(
  (
    { className, orientation = "horizontal", size = "2", children, ...props },
    ref,
  ) => {
    const isVertical =
    ...
    return (
      <dl
        ...
        className={cn(
          ...
          isVertical
            ? "flex flex-col gap-3"
            : "grid grid-cols-[max-content_1fr] gap-x-4 gap-y-3",
          className,
        )}
        {...props}
      >
...
const DataListItem = React.forwardRef<HTMLDivElement, DataListItemProps>(
  ({ className, align, children, ...props }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={cn("flex flex-wrap", alignClass, className)}
        {...props}
      >
        {children}
      </div>
  ...
  ({ className, heading, children, ...props }, ref) => {
    return (
      <div ref={ref} className={cn("space-y-2", className)} {...props}>
        {heading && <h3 className="font-semibold text-sm">{heading}</h3>}
