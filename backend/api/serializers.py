...
class TechniqueAttributeSerializer(serializers.ModelSerializer):
    attribute_type = serializers.ReadOnlyField(
        source="attribute_value.attribute_type.name"
    )
...
class TechniqueWriteSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        try:
            ...
            if (
                assurance_goal_ids is not None
                or category_ids is not None
                or subcategory_ids is not None
    ...
    def _add_relationships(
        ...
        for goal_id in assurance_goal_ids:
            TechniqueAssuranceGoal.objects.create(
                technique=technique, assurance_goal_id=goal_id
            )

        # Add categories
        for category_id in category_ids:
            TechniqueCategory.objects.create(
                technique=technique, category_id=category_id
            )

        # Add subcategories
        for subcategory_id in subcategory_ids:
            TechniqueSubCategory.objects.create(
                technique=technique, subcategory_id=subcategory_id
            )

        # Add tags
        for tag_id in tag_ids:
            TechniqueTag.objects.create(technique=technique, tag_id=tag_id)
            ...
            for attr_data in attributes_data:
                ...
                if attribute_value_id:
                    TechniqueAttribute.objects.create(
                        technique=technique, attribute_value_id=attribute_value_id
                    )
            ...
            for limitation_text in limitations_data:
                if limitation_text:
                    TechniqueLimitation.objects.create(
                        technique=technique, description=limitation_text
                    )
