...
test.describe("Critical User Journeys", () => {
  ...
  test.describe("Browse Techniques Journey", () => {
    test("should allow user to browse and view technique details", async ({
      page,
    }) => {
      ...
      await expect(
        page.locator('[data-testid="techniques-grid"]'),
      ).toBeVisible();
      ...
      const firstTechniqueCard = page
        ...
        .first();
      const firstTechniqueName = await firstTechniqueCard
        .locator("h3")
        .textContent();
      ...
      await expect(
        page.locator('[data-testid="technique-description"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="technique-ratings"]'),
      ).toBeVisible();

      // Verify assurance goals and tags are displayed
      await expect(
        page.locator('[data-testid="assurance-goals"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="technique-tags"]'),
      ).toBeVisible();
    ...
    test("should display technique acronym when available", async ({
      page,
    }) => {
      // Look for techniques with acronyms (like SHAP, LIME)
      const shapeCard = page.locator(
        '[data-testid="technique-card-shapley-additive-explanations"]',
      );
      if (await shapeCard.isVisible()) {
        await expect(shapeCard.locator("text=SHAP")).toBeVisible();
      }

      const limeCard = page.locator(
        '[data-testid="technique-card-local-interpretable-model-agnostic-explanations"]',
      );
  ...
  test.describe("Search and Filter Journey", () => {
    ...
    test("should allow user to filter by complexity rating", async ({
      page,
    }) => {
      // Select complexity filter
      const complexityFilter = page.locator(
        'select[aria-label*="complexity" i]',
      );
    ...
    test("should show no results message when no techniques match", async ({
      page,
    }) => {
  ...
  test.describe("Create Technique Journey", () => {
    test("should allow user to create a new technique", async ({ page }) => {
      ...
      await page.fill(
        ...
        "This is a test technique created during E2E testing to verify the form functionality works correctly.",
      );
