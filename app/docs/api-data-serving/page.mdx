...
TEA Techniques provides programmatic access to its data through a collection of static JSON files hosted on GitHub Pages. While not a traditional API with a server processing requests, this approach allows users or developers to fetch technique data, and filtered collections using standard HTTP requests.

Developers can integrate TEA Techniques data into their applications just as they would with any REST API. The files are served with proper CORS headers, cached by GitHub's CDN, and available without authentication.

This documentation outlines the available endpoints, data structures, and best practices for consuming the TEA Techniques data in your projects.
...
During the build process, our data generation scripts transform the master `techniques.json` file into:
...
<Table>
  ...
  <TableBody>
    <TableRow>
      ...
      <TableCell><Badge variant="outline">~429KB</Badge></TableCell>
...
```
GET /data/filters/[category]/[tag].json
```
...
Goal-specific search indices for scoped searching:

```
GET /data/search/[goal]-index.json
```

These provide optimised Fuse.js indices for searching within specific assurance goals.
