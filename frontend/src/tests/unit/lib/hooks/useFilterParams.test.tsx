...
describe('useFilterParams', () => {
  ...
  describe('initialization', () => {
    it('should initialize with default filters when no URL params', () => {
      ...
      const { result } = renderHook(() => 
        useFilterParams(initialFilters, 1)
      );
  ...
  describe('updateFilter', () => {
    it('should update a single filter and reset page to 1', () => {
      ...
      expect(result.current.currentPage).toBe(1);
      expect(mockPush).toHaveBeenCalledWith('/techniques?search=new+search&page=1');
    });
    ...
    it('should handle special plural parameter mapping', () => {
      ...
      expect(result.current.filters.assurance_goal).toBe('fairness');
      expect(mockPush).toHaveBeenCalledWith('/techniques?assurance_goals=fairness&page=1');
    });

    it('should handle tag to tags mapping', () => {
      const { result } = renderHook(() => 
        useFilterParams({ tag: '' }, 1)
      );
      ...
      expect(result.current.filters.tag).toBe('machine-learning');
      expect(mockPush).toHaveBeenCalledWith('/techniques?tags=machine-learning&page=1');
    });
  });
  ...
  describe('updateMultipleFilters', () => {
    it('should update multiple filters at once', () => {
      ...
      expect(result.current.currentPage).toBe(1);
      expect(mockPush).toHaveBeenCalledWith('/techniques?search=test+query&tags=ml&page=1');
    });
  ...
  describe('setPage', () => {
    ...
    it('should handle page navigation with existing filters', () => {
      ...
      expect(mockPush).toHaveBeenCalledWith('/techniques?search=test&tags=ml&page=3');
    });
  });
  ...
  describe('URL parameter encoding', () => {
    it('should properly encode spaces in filter values', () => {
      ...
      expect(mockPush).toHaveBeenCalledWith('/techniques?search=machine+learning&page=1');
    });
    ...
    it('should handle special characters in filter values', () => {
      ...
      expect(mockPush).toHaveBeenCalledWith('/techniques?search=test+%26+validation&page=1');
    });
  });
