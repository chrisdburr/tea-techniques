...
describe("useTechniques Hook", () => {
  ...
  describe("Search and Filtering", () => {
    it("applies search filters correctly", async () => {
      ...
      const { result } = renderHook(
        () => useTechniques({ search: searchTerm }),
        {
          wrapper: createWrapper(),
        },
      );
      ...
      expect(
        result.current.data?.results.some(
          (t) => t.name.includes("SHAP") || t.acronym?.includes("SHAP"),
        ),
      ).toBe(true);
    });
    ...
    it("applies assurance goal filters", async () => {
      ...
      const { result } = renderHook(
        () => useTechniques({ assurance_goals: [goalId] }),
        {
          wrapper: createWrapper(),
        },
      );
      ...
      expect(
        result.current.data?.results.every((t) =>
    ...
    it("applies complexity rating filters", async () => {
      ...
      const { result } = renderHook(
        () =>
          useTechniques({
            complexity_min: complexityRating.toString(),
            complexity_max: complexityRating.toString(),
          }),
        {
          wrapper: createWrapper(),
        },
      ...
      expect(
        result.current.data?.results.every(
          (t) => t.complexity_rating === complexityRating,
        ),
      ).toBe(true);
    });
  ...
  describe("Pagination", () => {
    it("handles pagination parameters", async () => {
      const { result } = renderHook(
        () => useTechniques({ page: 2, page_size: 10 }),
        {
          wrapper: createWrapper(),
        },
      );
  ...
  describe("Caching and Performance", () => {
    it("caches results for identical queries", async () => {
      ...
      // First render
      const { result: result1 } = renderHook(() => useTechniques(), {
        wrapper,
      });
      ...
      const { result: result2 } = renderHook(() => useTechniques(), {
        wrapper,
      });
...
describe("useTechniqueDetail Hook", () => {
  ...
  describe("Related Techniques", () => {
    it("fetches related techniques using slugs", async () => {
      ...
      if (technique.related_techniques.length > 0) {
        expect(
          technique.related_techniques.every((rt) => typeof rt === "string"),
        ).toBe(true);
      }
    });
  });
  ...
  describe("Caching Behavior", () => {
    it("caches technique details", async () => {
      ...
      // First render
      const { result: result1 } = renderHook(
        () => useTechniqueDetail(techniqueSlug),
        { wrapper },
      );
    ...
    it("maintains separate cache entries for different techniques", async () => {
      ...
      const { result: result1 } = renderHook(() => useTechniqueDetail(slug1), {
        wrapper,
      });
      ...
      const { result: result2 } = renderHook(() => useTechniqueDetail(slug2), {
        wrapper,
      });
