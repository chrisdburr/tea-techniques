...
class AssuranceGoalAPITests(BaseAPITestCase):
    ...
    def test_create_assurance_goal(self):
        ...
        data = {
            'name': 'New Test Goal',
            'description': 'A newly created test goal'
        }
        ...
        # Verify database
        self.assertTrue(
            AssuranceGoal.objects.filter(name='New Test Goal').exists()
        )
    ...
    def test_delete_assurance_goal(self):
        ...
        self.assertFalse(
            AssuranceGoal.objects.filter(id=goal.id).exists()
        )
...
class TechniqueAPITests(BaseAPITestCase):
    ...
    def test_retrieve_technique(self):
        ...
        use_case = TechniqueExampleUseCaseFactory(
            technique=technique,
            assurance_goal=self.assurance_goals[0]
        )
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['id'], technique.id)
    ...
    def test_create_technique_minimal(self):
        ...
        # Verify database
        technique = Technique.objects.get(id=response.data['id'])
    ...
    def test_delete_technique(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertFalse(Technique.objects.filter(id=technique.id).exists())
...
class APIErrorHandlingTests(BaseAPITestCase):
    ...
    def test_404_error_handling(self):
        ...
        url = reverse('technique-detail', kwargs={'pk': 99999})
        response = self.client.get(url)
...
class APIPaginationTests(BaseAPITestCase):
    ...
    def test_page_navigation(self):
        ...
        first_ids = [r['id'] for r in first_page_results]
        second_ids = [r['id'] for r in second_page_results]
...
class APIFilteringTests(BaseAPITestCase):
    ...
    def setUp(self):
        ...
        self.high_complexity = TechniqueFactory(
            name='High Complexity Technique',
            complexity_rating=5
        )
        self.low_complexity = TechniqueFactory(
            name='Low Complexity Technique',
            complexity_rating=1
        )
