...
export const accessibilityUtils = {
  ...
  testTabOrder: async (
    ...
    const focusableElements =
      accessibilityUtils.findFocusableElements(container);

    for (let i = 0; i < expectedOrder.length; i++) {
      const expectedElement = container.querySelector(
        `[data-testid="${expectedOrder[i]}"], [aria-label="${expectedOrder[i]}"]`,
      ) as HTMLElement;

      if (!expectedElement) {
        throw new Error(
          `Expected element with testid or aria-label "${expectedOrder[i]}" not found`,
        );
      }
      ...
      if (actualIndex !== i) {
        throw new Error(
          `Tab order mismatch: expected ${expectedOrder[i]} at position ${i}, but found at position ${actualIndex}`,
        );
      }
    }
  },
...
export const componentAccessibilityTests = {
  ...
  button: async (renderResult: RenderResult, buttonTestId: string) => {
    ...
    if (!accessibilityUtils.hasAccessibleName(button)) {
      throw new Error(
        "Button must have an accessible name (aria-label, text content, or title)",
      );
    }
  ...
  modal: async (renderResult: RenderResult, modalTestId: string) => {
    ...
    if (!accessibilityUtils.hasAccessibleName(modal)) {
      throw new Error(
        "Modal must have an accessible name (aria-label or aria-labelledby)",
      );
    }
