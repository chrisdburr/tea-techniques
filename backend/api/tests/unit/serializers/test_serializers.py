...
class AssuranceGoalSerializerTests(TestCase):
    ...
    def test_update_assurance_goal(self):
        ...
        self.assertEqual(
            ...
            "Updated description for explainability techniques",
        )
...
class ResourceTypeSerializerTests(TestCase):
    ...
    def test_deserialize_valid_resource_type(self):
        ...
        data = {"name": "GitHub Repository", "icon": "github_repository"}
...
class TechniqueResourceSerializerTests(TestCase):
    ...
    def test_validate_url_valid_urls(self):
        ...
        serializer = TechniqueResourceSerializer()
        ...
        for url in valid_urls:
            with self.subTest(url=url):
                result = serializer.validate_url(url)
    ...
    def test_validate_url_edge_cases(self):
        ...
        result = serializer.validate_url(long_url)
        self.assertEqual(result, long_url)

        # Test URL with special characters
        special_url = "https://example.com/path?param=value&other=test#fragment"
        result = serializer.validate_url(special_url)
        self.assertEqual(result, special_url)

        # Test international domain (Note: may not pass with current regex pattern)
        # The current regex doesn't support Unicode domains, so test a valid ASCII domain
        intl_url = "https://example.co.jp"
        result = serializer.validate_url(intl_url)
        self.assertEqual(result, intl_url)
...
class TechniqueExampleUseCaseSerializerTests(TestCase):
    ...
    def test_get_assurance_goal_name_method(self):
        ...
        # Test without goal
        use_case_without_goal = TechniqueExampleUseCaseFactory(assurance_goal=None)
...
class TechniqueLimitationSerializerTests(TestCase):
    ...
    def test_deserialize_valid_limitation(self):
        ...
        data = {"technique": technique.slug, "description": "Test limitation"}
...
class TechniqueSerializerTests(TestCase):
    ...
    def test_serialize_technique_with_relationships(self):
        ...
        self.assertEqual(data["related_techniques"][0], self.related_technique.slug)
    ...
    def test_writable_id_fields_present(self):
        ...
        self.assertIn("related_technique_slugs", serializer.fields)
        ...
        self.assertTrue(serializer.fields["related_technique_slugs"].write_only)
    ...
    def test_validation_with_invalid_ids(self):
        ...
        data = {
            ...
            "related_technique_slugs": ["non-existent-slug"],  # Non-existent slug
        }
        ...
        self.assertIn("related_technique_slugs", errors)
    ...
    def test_validation_with_valid_ids(self):
        ...
        data = {
            ...
            "assurance_goal_ids": [self.assurance_goal.id],
            "tag_ids": [self.tag.id],
            "related_technique_slugs": [self.related_technique.slug],
        }
    ...
    def test_technique_serialization_includes_slug_and_acronym(self):
        ...
        technique = IsolatedTechniqueFactory(name="Test Technique", acronym="TT")

        serializer = TechniqueSerializer(technique)
        data = serializer.data

        self.assertIn("slug", data)
        self.assertIn("acronym", data)
        self.assertEqual(data["slug"], technique.slug)
        self.assertEqual(data["acronym"], "TT")

    def test_technique_serialization_with_null_acronym(self):
        """Test that technique serialization handles null acronym correctly."""
        technique = IsolatedTechniqueFactory(name="Test Technique", acronym=None)

        serializer = TechniqueSerializer(technique)
        data = serializer.data

        self.assertIn("acronym", data)
        self.assertIsNone(data["acronym"])

    def test_technique_create_with_slug_validation(self):
        """Test that technique creation validates slug field."""
        data = {
            "slug": "test-technique-slug",
            "name": "Test Technique",
            "description": "Test description",
            "assurance_goal_ids": [self.assurance_goal.id],
        }
    ...
    def test_technique_create_with_acronym(self):
        """Test that technique creation handles acronym field."""
        data = {
            "slug": "acronym-technique",
            "name": "Acronym Technique",
            "acronym": "AT",
            "description": "Test description",
            "assurance_goal_ids": [self.assurance_goal.id],
        }
